=head1 NAME
    ...
    MetaScheduler::Client::status

=head1 DESCRIPTION

    Object for handling task status requests to the scheduler
    Should be called from the client script and gets it's
    configuration from the @ARGV parameters

=head1 SYNOPSIS

    use MetaScheduler::Client::status;
    MetaScheduler::Client::status->initialize($host, $port, @ARGV);
...
package MetaScheduler::Client::status;
